 ...
 * Windows native port for Fight the Machine project
 */
...
// Copyright (C) 2025 by Fight the Machine Project (Windows port)
...
static const char* WIN32_BLACKLIST[] = {
    ...
    /* Windows Shell */
    "explorer.exe", "searchui.exe", "startmenuexperiencehost.exe",
...
static void win32_get_process_user(DWORD pid, char *username, size_t size) {
    ...
    strncpy(username, "SYSTEM", size - 1);
    username[size - 1] = '\0';
...
/* Check if process is a service/daemon (running as SYSTEM or no visible window) */
static int win32_is_daemon(DWORD pid, const char *username) {
    /* Services typically run as SYSTEM, LOCAL SERVICE, or NETWORK SERVICE */
    if (_stricmp(username, "SYSTEM") == 0 ||
        _stricmp(username, "LOCAL SERVICE") == 0 ||
        _stricmp(username, "NETWORK SERVICE") == 0) {
        return 1;
    }
    return 0;
}
...
// ******************** WINDOWS ********************
    ...
    if (hSnap == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "ERROR: pr_check could not create process snapshot (error %lu)\n",
                GetLastError());
        return;
    }
    ...
    if (!Process32First(hSnap, &pe32)) {
        fprintf(stderr, "ERROR: pr_check Process32First failed (error %lu)\n",
                GetLastError());
        CloseHandle(hSnap);
        return;
    }
    ...
    do {
        ...
        if (procpid == 0 || procpid == GetCurrentProcessId()) {
            continue;
        }
        ...
        win32_get_process_user(procpid, username, sizeof(username));

        /* Determine if it's a daemon (service) */
        demon = win32_is_daemon(procpid, username);

        /* Check username filtering */
        if ((psallusers || in_ps_userlist(psuser_list_head, username)) &&
            !(in_ps_userlist(psnotuser_list_head, username))) {
            add_to_pid_list(procpid, pe32.szExeFile, demon);
        }
...
void pr_kill(int pid) {
  ...
  // Get process name for logging and blacklist check
  hSnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
  if (hSnap != INVALID_HANDLE_VALUE) {
      pe32.dwSize = sizeof(PROCESSENTRY32);
      if (Process32First(hSnap, &pe32)) {
  ...
  if (!hProc) {
      fprintf(stderr, "pr_kill: Cannot open process %d '%s' (error %lu)\n",
              pid, procname, GetLastError());
      return;
  }
  ...
  if (TerminateProcess(hProc, 1)) {
      ...
      fprintf(stderr, "pr_kill: TerminateProcess failed for '%s' PID %d (error %lu)\n",
              procname, pid, GetLastError());
  }

  CloseHandle(hProc);

#else
  /* Unix implementation using system("kill") */
  char buf[256];
  char *cmd;
// If -nopsact was on the command line, don't actually kill the process
  if ( nopsact ){
...
void pr_renice(int pid) {
#if defined(_WIN32) || defined(_WIN64)
  /* Windows implementation using SetPriorityClass */
  HANDLE hProc;
  ...
  // Degrade priority one level (like renice +5)
...
#else
  /* Unix implementation using system("renice") */
  char buf[256];
  char *cmd;
